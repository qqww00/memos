...
export type SidebarRouteKind = MemoScope | "profile" | "views" | "boards" | "attachments" | "inbox" | "settings" | "memo" | "empty";
...
export const getSidebarRouteKind = (path: string): SidebarRouteKind => {
  ...
  if (path === ROUTES.BOARDS || matchPath("/boards/:boardId", path)) return "boards";
